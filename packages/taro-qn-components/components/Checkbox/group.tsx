...
export default function Group(props) {
  const { children, defaultValue, value, options, onChange, disabled, layout, style, itemStyle } = props;
  ...
  function renderChild(_children) {
    if (typeof options !== 'undefined') {
      return getOptions().map((option) => {
        return (
          <CCheckbox disabled={option.disabled} checked={currentValue.indexOf(option.value) !== -1} value={option.value} key={option.value} style={itemStyle} onChange={checkboxChange}>
            {option.label}
          </CCheckbox>
        );
      });
  ...
  const checkboxes = useMemo(() => renderChild(children), [options, currentValue, disabled, children]);
